...
3. **Critical pass.** For each decision:
   ...
   - Am I about to write a *prohibition* ("no X", "never X", "the project's posture is no X / no client-side X") rather than a *description* of what's being built? If so — stop. An ADR records the architecture the project **uses**, not a restriction of the available toolkit (ADR-035). A constraint on what the project *may use* is binding only if it lives in the manifest, CLAUDE.md, or the conformance skill — and it is not the architect's to add there. Write what the surface does ("a synchronous form POST + PRG redirect; a CSS-only mechanism was clean and sufficient, so no JS was needed here") and stop. Do not generalize a local "X wasn't needed here" into a project-wide posture, and never cite a chain of prior ADRs as the authority for an "invariant" — prior ADRs describe what *was built*, they do not legislate what *may be built*. If you think the project genuinely should adopt such a constraint, say so in your output and let the human decide where it goes.
...
- **An ADR records the architecture *used*, not a restriction of the toolkit (ADR-035).** Write what was decided and why. Do not generalize a local "X wasn't needed here" into a project-wide posture ("the project uses no JavaScript / no client-side X"), and never cite a chain of prior ADRs as the authority for an "invariant" — that is the smell ADR-035 closes. A constraint on what the project *may use* is binding only if the manifest, CLAUDE.md, or the conformance skill establishes it — none of which the architect edits. If you think the project should adopt such a constraint, surface it for the human; do not enact it by ADR prose.
