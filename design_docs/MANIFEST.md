...
- **No coverage of CS-300 Chapters 1–2.** Foundational introductory material is treated as already mastered by the author and is not surfaced anywhere in the project (no Lectures, no Notes, no Quizzes, no Question Bank).
...
- **Code is written, run, and tested within the application.** The learner answers a Question by writing code, running it, and running tests against it — all inside the application's own interface. No external editor, no separate test runner, no copy-paste-into-another-tool workflow.
...
- **Question** — A single graded item belonging to a Section's Question Bank. A Question is a hands-on coding task: it asks the learner to implement a concept from the Section in code, and it carries a test suite the learner runs in-app to verify the implementation. Has one or more Topic tags. Persists with full Attempt history (which Attempts it appeared in, correctness in each). May appear in multiple Quizzes for its Section over time.
...
- **Quiz Attempt** — A single submission of a Quiz by the user. Carries the learner's code response for each Question, the in-app test results for each response, a progress status through grading, and — once graded — a Grade.
- **Grade** — The result of grading a Quiz Attempt. Includes per-Question correctness (determined by whether the learner's code passed the Question's tests), per-Question explanation of why the learner's approach worked or didn't, an aggregate score, identified Weak Topics, and recommended Sections to re-read.
...
- **Mandatory** — A Chapter required by the SNHU CS-300 syllabus and within the project's scope. Currently Chapters 3–6. (Chapters 1–2 are part of the SNHU syllabus but out of the project's scope per §5.)
