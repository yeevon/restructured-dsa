...
**Superseded by:** `ADR-031` (§Decision only — the no-fragment redirect mechanism. ADR-030's load-bearing principle, its ADR-025 bookkeeping, and the no-JS form-handling shape it stayed within are carried forward by ADR-031 — note that shape was never a project rule; see ADR-035.)
...
- The no-JS form-handling shape (ADR-023 / ADR-025 / ADR-027 / ADR-028) is **followed** — Option 1 introduces no client-side code; it is a one-line route-handler change. (Following a clean recipe, not honoring a project rule — see ADR-035.)
...
- The human's framing — *"or just no anchor, radical idea i know"* — reads as an explicit preference for the *smallest* change that respects the principle. Option 1 *is* that change: one route-handler line, zero new code surface, and it stays within the no-JS form-handling shape the Notes and completion surfaces already use.
...
- Option 2 would be the **first JavaScript in the project** — a sizable step (a client-side asset + a behavioral surface + JS-on/JS-off test variants) far larger than the immediate feature. JavaScript is part of the available toolkit (see ADR-035), but reaching for it to solve a problem Option 1 solves with a one-line change would be disproportionate. (Reframing note: earlier wording here read as if avoiding JS were a standing prohibition the prior ADRs established — it never was; the prior ADRs simply found a no-JS recipe clean and sufficient for their surfaces.)
...
- **Form-handling pattern.** Synchronous PRG; no JavaScript needed for this surface; `303 See Other` on success; idempotent mark/unmark semantics (`mark` against an already-complete Section is a no-op; `unmark` against an already-incomplete Section is a no-op). Unchanged — only the redirect *target* drops the fragment.
...
Considered as the *fallback*, not the design. The cleanest UX (zero scroll motion, no flicker, no round-trip; the rail count updates instantly) — but it would be the **first JavaScript in the project**, a sizable step (a client-side asset + a behavioral surface + JS-on/JS-off test variants) larger than the immediate feature. **Rejected as the design** because Option 1 solves the problem with one route-handler line and a Playwright assertion, no client-side code needed, and matches the human's "smallest change that respects the principle" framing. **Retained as the contingency** only if the test-writer/implementer phase demonstrates Chromium does not preserve scroll on the fragment-less same-URL navigation — and even then, via its own ADR deciding where the JS lives and how it's tested, gated by the human, never silently. (Per the task file's "Architectural concerns": "try Option 1 first … only fall back to Option 2 if Option 1 demonstrably fails the Playwright scroll-preservation assertion; if Option 2, write a separate, dedicated ADR for the JS, mark it `Status: Proposed`, and end `/design` with `> NEEDS HUMAN`." This `/design` cycle picks Option 1, so that ADR is not written and `> NEEDS HUMAN` is not raised.) (Reframing note: "deliberately avoided by ADR-023/025/027/028" in the original wording over-stated — those ADRs didn't *avoid* JS as a matter of policy; they reached clean no-JS solutions for their surfaces. JavaScript was always part of the toolkit; see ADR-035.)

**D. Option 3 from the project_issue: JS-required async, no PRG fallback.**

Rejected. Same client-side-JS cost as Option 2, plus it removes the working no-JS path without the progressive-enhancement excuse: the `<form>` becomes a `<button type="button">` that only works via JS, the no-JS case stops existing (and stops being testable), and the project's "trust the simplest mechanism" preference is rejected without strong reason. If the project adopts client-side JS here, it should be progressive enhancement (Option 2), not JS-required (Option 3). (The objection is to JS-*required* over progressive-enhancement, not to client-side JS as such — see ADR-035.)
...
- The no-JS form-handling shape (ADR-023 / ADR-025 / ADR-027 / ADR-028) — **followed by this ADR**: Option 1 introduces no client-side code. (Adding client-side JS would be its own ADR if a real need arises; that's a normal architectural step, not a forbidden threshold — see ADR-035. This `/design` cycle writes no such ADR.)
...
- **MC-1 (No direct LLM/agent SDK use).** Orthogonal — completion has no AI surface. Note: this ADR introduces **no JavaScript at all** (Option 1); even the "is client JS an MC-1 matter?" question (it is not — MC-1 governs AI SDKs, not client JS) does not arise here. Under the Option 2 contingency, MC-1 would still be irrelevant — adding client-side JS is a normal architectural step (its own ADR), not an MC-1 matter, and not the breach of any project-wide rule (there is none; see ADR-035).
